...
public class CustomerBankInfoAppServiceTests
{
    ...
    {
        // Act
        var actionResult = () => _customerBankInfoAppService.AccountBalanceIsBiggerThanZero(It.IsAny<int>());

        // Assert
        actionResult.Should().NotThrow();
        _customerBankInfoServiceMock.Verify(x => x.AccountBalanceIsBiggerThanZero(It.IsAny<int>()), Times.Once);
    }
